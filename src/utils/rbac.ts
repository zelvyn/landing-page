...
export const ROUTE_PERMISSIONS: RoutePermission[] = [
  ...
  { path: "/explore", allowedRoles: ["ARTIST", "USER", "GUEST"], requireAuth: false },
  { path: "/artists", allowedRoles: ["ARTIST", "USER", "GUEST"], requireAuth: false },
  { path: "/about", allowedRoles: ["ARTIST", "USER", "GUEST"], requireAuth: false },
  { path: "/terms", allowedRoles: ["ARTIST", "USER", "GUEST"], requireAuth: false },
  { path: "/privacy", allowedRoles: ["ARTIST", "USER", "GUEST"], requireAuth: false },
  
  // Auth routes (only for non-authenticated users)
  { path: "/auth/login", allowedRoles: ["GUEST"], requireAuth: false },
  { path: "/auth/signup", allowedRoles: ["GUEST"], requireAuth: false },
  { path: "/auth/forgot-password", allowedRoles: ["GUEST"], requireAuth: false },
  ...
  { path: "/notifications", allowedRoles: ["ARTIST", "USER"], requireAuth: true },
];
...
export const checkRoutePermission = (
  ...
  const permission = ROUTE_PERMISSIONS.find(p => 
    p.path === path || (p.path.endsWith("/*") && path.startsWith(p.path.slice(0, -2)))
  );
...
export const getRedirectPath = (userRole: UserRole, isAuthenticated: boolean): string => {
